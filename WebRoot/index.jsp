...
<!--文件体开始-->
		...
		<%int i=1; %>
       <c:forEach items="${applicationScope.products}" var="entry">      
		<tr>
			<td class=tablebody2 valign=middle align=center width=""><%=i++ %></td>
			<td class=tablebody1 valign=middle width="">&nbsp;&nbsp;<a href="productDetailSer?id=${entry.key}">${entry.value.name}</a></td>
			<td class=tablebody2 valign=middle align=center width="">${entry.value.price}</td>
			<td class=tablebody1 valign=middle align=center width=""><a href="user/addProductSer?id=${entry.key}">
